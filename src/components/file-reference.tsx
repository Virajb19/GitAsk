           ...
           <Tabs value={tab} onValueChange={setTab}>
                <div className='flex gap-2 bg-gray-800 overflow-scroll rounded-md p-2'>
                   {files.map((file,i) => {
                    ...
                    tab === file.filename ? 'text-primary-foreground bg-sidebar-primary-foreground' : 'text-muted-foreground border border-zinc-600 hover:text-primary hover:border-transparent hover:bg-gray-700')} key={i}>{file.filename}</button>
                   })}
                </div>
                {files.map(file => {
